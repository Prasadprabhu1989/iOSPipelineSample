def xcodeproj = 'iOSPipelineSample.xcodeproj' // Path to the xcodeproj
def xcarchive_name = "iOSPipelineSample.xcarchive" // Name of the archive to build
def build_scheme = 'iOSPipelineSample' // Scheme to build the app
def test_scheme = 'iOSPipelineSample' // Scheme to build tests
...
stage('Test') {
				steps{
					sh 'xcodebuild -project iOSPipelineSample/iOSPipelineSample.xcodeproj -scheme "iOSPipelineSample" -configuration "Debug" build test -destination "platform=iOS Simulator,name=iPhone 11 Pro Max,OS=13.4.1" -enableCodeCoverage YES clean test | /usr/local/bin/ocunit2junit'
step([$class: 'JUnitResultArchiver', allowEmptyResults: true, testResults: '**/test-reports/*.xml, healthScaleFactor: 1.0, fingerprint: true'])
	...
	thinning: '', 
	xcodeProjectFile: '', 
	xcodeProjectPath: 'iOSPipelineSample', 
	xcodeSchema: 'iOSPipelineSample', 
	xcodeWorkspaceFile: '', 
...
//xcodebuild -exportArchive -archivePath  $WORKSPACE/build/xcarchive  -exportPath "${WORKSPACE}/build/ipa"  -exportOptionsPlist  "${WORKSPACE}/build/plist"
 			//sh 'xcrun altool --upload-app --type ios --file $WORKSPACE/iOSPipelineSample/build/Release-iphoneos/*.ipa --username "rajendrarao" --password "Coolraja1978@1"'
...
//step([$class: 'JUnitResultArchiver', allowEmptyResults: true, testResults: '**/test-reports/*.xml, healthScaleFactor: 1.0, fingerprint: true'])
//archiveArtifacts artifacts: '$WORKSPACE/iOSPipelineSample/build/Release-iphoneos/*.ipa', fingerprint: true
